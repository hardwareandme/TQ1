...
try:
    ...
    submit_comment = browser.find_element_by_name("submit")
    submit_comment.click()
    time.sleep(30)
    #check if user is posting comments frequently
    if "You are posting comments too quickly. Slow down." not in browser.page_source:
        if "Duplicate comment detected" not in browser.page_source:
            #Assert only if above two condition pass
            assert "comment by User2" in browser.page_source
            print "comment 2 is added as a reply to comment 1"
        else:
            print "Duplicate comment detected\n"
    else:
        print "You are posting comments too quickly. Slow down."
except:
